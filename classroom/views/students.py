...
from ..serializers import QuizSerializer, TakenQuizSerializer, RandomQuizSerializer, QuestionSerializer, \
    StudentAnswerSerializer, SubjectSerializer, AnswerSerializer
...
class AllQuizzes(generics.ListAPIView):
    ...
    def get_queryset(self):
        return Quiz.objects.select_related('subject').all() \
...
class TakenQuizzes(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return TakenQuiz.objects.select_related('student').filter(student_id=candidate_id)
...
class QuizQuestions(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Question.objects.select_related('quiz').filter(quiz_id=quiz_id)


class QuestionAnswer(generics.ListAPIView):
    serializer_class = AnswerSerializer

    def get_queryset(self):
        question_id = self.kwargs['question_id']
        return Answer.objects.select_related('question').filter(question_id=question_id)
...
class TakeQuiz(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        questionlist = []
        quizzes = RandomQuiz.objects.select_related('student').filter(student_id=student.id).filter(quiz_id=quiz_id)
        if len(quizzes) > 0:
            return quizzes
        else:
...
class DeleterQuizRandom(generics.ListAPIView):
    permission_classes = (IsAuthenticated,)
    serializer_class = RandomQuizSerializer
    ...
    def get_queryset(self):
        quiz_id = self.kwargs['quiz']
        student_id = self.kwargs['student_id']
        RandomQuiz.objects.filter(quiz_id=quiz_id, student_id=student_id).delete()
...
def take(request, pk):
    ...
    if quiz.name in takenquizlist:
        ...
        if not tempquiz.questions == None:
                ...
                if form.is_valid():
                    with transaction.atomic():
                        if 'answer' in request.POST:
                            ...
                            default_answer = StudentAnswer(quiz=quiz, student=student,
                                                           answer=Answer.objects.filter(question_id=question.id).last())
            ...
            correctanswercounter = StudentAnswer.objects.filter(quiz=quiz, student=student,
