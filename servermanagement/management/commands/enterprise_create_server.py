...
from jobs.models import EnterpriseJob, Server, EnterpriseCandidateSetup, ServerConfig



def test_create_server(request=None):
    jobs = EnterpriseJob.objects.filter(has_executed=False,
                              type='create_server',
    ...
    for job in jobs:
        if datetime.datetime.now().hour - job.time.hour < 3:
            ...
            setup, created = EnterpriseCandidateSetup.objects.get_or_create(project_name=project_name,
                                                                            setup_code=setup_code, start_time=job.time,
                                                                            project=job.project)
            if created:
                ...
                if server.get_create_status():
                    ...
                    job.save()
                    EnterpriseJob.objects.create(type='create_workspace', project=job.project,
