...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        jobs = EnterpriseJob.objects.filter(has_executed=False, type='get_analysis')
        for job in jobs:
            ...
            candidate_name = f'{job.project.enterprise_candidate.candidate.first_name}-{job.project.enterprise_candidate.candidate.last_name}'
            ...
            EnterpriseJob.objects.create(type='clean_up', project=job.project,data={'setup_code': job.data['setup_code'],'candidate_id': job.data['candidate_id']})