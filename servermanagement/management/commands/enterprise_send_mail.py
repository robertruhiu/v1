...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        jobs = EnterpriseJob.objects.filter(has_executed=False, type='send_mail')
        for job in jobs:
            candidate =  EnterpriseCandidateSetup.objects.get(setup_code=job.data['setup_code'])
            # todo: uncomment during demo
            setup_finished_mail(job.project.enterprise_candidate.candidate.email, candidate.workspace_url)
            ...
            job.save()
            EnterpriseJob.objects.create(type='monitor_workspace',  project=job.project,
